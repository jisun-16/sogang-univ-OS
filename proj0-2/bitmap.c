...
struct bitmap* bitmap_expand(struct bitmap* bitmap, int size) {
    int original_size = bitmap_size(bitmap);
    unsigned long* exp = (unsigned long*)realloc(bitmap->bits, (size + original_size) * sizeof(unsigned long));
    
    if (exp == NULL)
        return NULL;

    bitmap->bits = exp;
    bitmap->bit_cnt = size + original_size;
    bitmap_set_multiple(bitmap, original_size, size, false);
